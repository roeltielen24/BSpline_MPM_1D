function [x, sol, vel] = exact_solution_X(rho, E, p0, g, H, x0,t )
...
part=zeros(length(x0),20);
vel_part=zeros(length(x0),20);
sol=zeros(length(x0),length(t));
vel=zeros(length(x0),length(t));
for l=1:length(t)
    for k=1:20
    ...
    sol(:,l) = C_x0 + sum(part,2);
    vel(:,l) = sum(vel_part,2); 
    clear part vel_part;
end

%Find the position
x = x0*ones(1,length(t))+sol;
end